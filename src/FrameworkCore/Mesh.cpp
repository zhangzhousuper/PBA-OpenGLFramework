...
BasicTriMesh::BasicTriMesh(const aiMesh *mesh)
    : vertices(mesh->mNumVertices), triangles(mesh->mNumFaces) {
...
std::vector<glm::vec3> BasicTriMesh::GetRealVertexNormals() {
  ...
  std::for_each(normals.begin(), normals.end(),
                [](glm::vec3 &vec) { vec = glm::normalize(vec); });
  return normals;
};
...
void BasicRenderTriMesh::BindVerticesAttributeSequence_() {
  ...
  size_t normalAttributeBeginOffset =
      verticeSizeOffset + offsetof(VertexAttribute, normalCoord);

  glEnableVertexAttribArray(1);
  glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(VertexAttribute),
                        reinterpret_cast<void *>(normalAttributeBeginOffset));

  size_t textureAttributeBeginOffset =
      verticeSizeOffset + offsetof(VertexAttribute, textureCoord);
...
void BasicRenderTriMesh::CopyAttributes_(const aiMesh *mesh) {
  std::ranges::iota_view vertexIDView{size_t(0), vertices.size()};
  std::for_each(
      vertexIDView.begin(), vertexIDView.end(), [mesh, this](size_t id) {
...
BasicRenderTriMesh::BasicRenderTriMesh(BasicRenderTriMesh &&another) noexcept
    : BasicTriMesh{std::move(another)}, verticesAttributes{std::move(
                                            another.verticesAttributes)},
...
BasicRenderTriMesh &
BasicRenderTriMesh::operator=(BasicRenderTriMesh &&another) noexcept {
...
BasicRenderTriMesh::~BasicRenderTriMesh() { ReleaseRenderResources_(); }

void BasicRenderTriMesh::Draw(Shader &shader) {
  int textureCnt = 0;
  auto SetTexture = [&textureCnt,
                     &shader](const std::string &namePrefix,
                              decltype(diffuseTextureRefs) textures) {
    int currTypeID = 1;
...
// TODO get the model center (bounding box center)
