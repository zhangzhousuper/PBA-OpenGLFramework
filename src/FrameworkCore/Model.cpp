...
BasicTriModel::BasicTriModel(const std::filesystem::path &modelPath) {
  ...
  const aiScene *model = importer.ReadFile(
      modelPath.string(), aiProcess_Triangulate | aiProcess_GenSmoothNormals |
                              aiProcess_FlipUVs |
...
void BasicTriModel::LoadResources_(const aiScene *model) {
  LoadResourcesDecorator_(model, [this](const aiMesh *mesh) {
...
void BasicTriRenderModel::LoadResources_(
    const aiScene *model, const std::filesystem::path &resourceRootPath,
    bool textureNeedFlip) {
  stbi_set_flip_vertically_on_load(textureNeedFlip);

  LoadResourcesDecorator_(
      model,
