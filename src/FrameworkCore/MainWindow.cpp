...
std::function<void(double, double)> MainWindow::s_mouseButtonCallback_;
...
void MainWindow::BindScrollCallback(
...
void MainWindow::BindCursorPosCallback(
    std::function<void(double, double)> callback) {
  if (callback) // if callback is not empty.
  {
...
void MainWindow::BindMouseButtonCallback(
    std::function<void(double, double)> callback) {
  if (callback) {
    s_mouseButtonCallback_ = callback;
    glfwSetMouseButtonCallback(
        window_, [](GLFWwindow *window, int button, int action, int mods) {
          s_mouseButtonCallback_(button, action);
        });
  } else {
    s_mouseButtonCallback_ = nullptr;
