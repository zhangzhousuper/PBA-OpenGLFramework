...
public:
  ...
  template <int keyCode> void BindKeyPressed(UpdateFunc &&func) {
    static bool lastPress = true;
    keyPressedList_.insert_or_assign(keyCode, [func, this]() {
  ...
  template <int keyCode> void BindKeyPressing(UpdateFunc &&func) {
    keyPressingList_.insert_or_assign(keyCode, [func, this]() {
      if (glfwGetKey(window_, keyCode) == GLFW_PRESS)
        std::invoke(func);
    });
    return;
  ...
  template <int keyCode> void BindKeyReleased(UpdateFunc &&func) {
    static bool lastRelease = true;
    keyReleasedList_.insert_or_assign(keyCode, [func, this]() {
  ...
  template <int keyCode> void BindKeyReleasing(UpdateFunc &&func) {
    keyReleasingList_.insert_or_assign(keyCode, [func, this]() {
      if (glfwGetKey(window_, keyCode) == GLFW_RELEASE)
        std::invoke(func);
    });
    return;
  ...
  void BindMouseButtonCallback(std::function<void(double, double)> callback);
...
private:
  ...
  static std::function<void(double, double)> s_mouseButtonCallback_;
